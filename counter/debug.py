...
def draw(predictions, image, image_name):
    ...
    for prediction in predictions:
        ...
        draw_image.text((xmin, max(0, ymin - 25)),label,font=font,fill=color)

    # Summary section at bottom of image
    if predictions:

        lowest_score = min(prediction.score for prediction in predictions)

        summary_text = (f"Total detections: {len(predictions)} | "f"Lowest confidence: {lowest_score:.3f}")

        draw_image.text((10, image_height - 30),summary_text,font=font,fill="blue")
